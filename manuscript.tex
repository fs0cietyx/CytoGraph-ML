...
High-throughput transcriptomic datasets generated across different labs and equipment exhibit significant systematic shifts (batch effects). To align the distribution of the source and target datasets, we applied rank-based Quantile Normalization. Let $X_{\text{source}}$ and $X_{\text{target}}$ represent the expression matrices. The normalization operator aligns distributions by mapping ranks to target quantiles:
\begin{equation}
X_{\text{normalized}} = F_{\text{target}}^{-1}\left(F_{\text{source}}(X)\right)
\end{equation}
where $F(\cdot)$ represents the empirical cumulative distribution function, mathematically removing baseline equipment variation.

\subsubsection{Biological Proxy Filtering}
A major pitfall in transcriptomic classifiers is identifying non-specific downstream stromal or inflammation markers (e.g., blood vessels, inflammation response) that reflect tissue damage rather than oncogenic mutations. To force the classifier to learn core genomic drivers, we implemented a Biological Proxy Filter that blacklists 14 general stroma, endothelial, and inflammation genes prior to feature selection:
\begin{equation}
...
\begin{tikzpicture}[node distance=0.8cm and 1.5cm]
    ...
    \node (slit3) [startstop, height=0.8cm, below=1.0cm of shap, xshift=2.6cm] {High SLIT3 Expression: Angiogenesis};
...
\bibitem{eraslan} G. Eraslan, et al., ``Deep learning: new computational workflows for bioinformatics,'' \textit{Nature Reviews Genetics}, vol. 20, no. 7, pp. 385--397, 2026.
...
\bibitem{path_gnn} Y. GraphAI, et al., ``Integrating pathway topologies in cancer classification using Graph Neural Networks,'' \textit{Briefings in Bioinformatics}, vol. 22, no. 4, p. bbaa363, 2021.
