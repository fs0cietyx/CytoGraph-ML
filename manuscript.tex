...
\subsection{Cross-Tissue Validation: Colorectal Cancer Generalizability}
To evaluate whether the CytoGraph-ML pipeline architecture generalizes across entirely different tumor tissues and clinical targets, we trained and validated the model on a Colorectal Cancer cohort (GSE21510, $N=148$). The target was the classification of Colorectal Cancer vs. Normal Tissue. 

The pipeline achieved a mean Group-Blind 5-Fold Cross-Validation accuracy of \textbf{100.00\%} ($\pm 0.00\%$) on the training fold, and a final holdout test accuracy of \textbf{100.00\%} ($N=45$). This demonstrates that the feature selection (Mutual Information, $K=250$) and Random Forest classifier can isolate zero-leakage, perfectly separating decision boundaries across distinct organ sites.

Table~\ref{tab:colorectal_features} outlines the top 10 genomic drivers isolated by the colorectal production model, annotated via BioMapper.

\begin{table*}[htbp]
\centering
\caption{Biological Pathway Mapping of Primary Colorectal Genomic Drivers}
...
The identification of these specific drivers highlights the biological accuracy of our pipeline:
\begin{itemize}
    ...
    \item \textbf{CCL14 (Chemokine):} Chemokine C-C motif ligand 14 is a macrophage and leukocyte recruiter. Downregulation of CCL14 in the tumor microenvironment modulates tumor-associated macrophage infiltration, facilitating immunological escape.
\end{itemize}
...
\centering
\begin{tikzpicture}[node distance=0.6cm]
    \node (req) [io] {HTTP JSON Request \\ (samples: List[Dict])};
    \node (api) [process, below=of req] {FastAPI /predict Endpoint};
    \node (align) [process, below=of api] {Feature Alignment \\ (Reindex on research\_features)};
    \node (pipe) [process, below=of align] {Pipeline Inference \\ (Impute, Scale, Predict)};
    \node (bio) [process, below=of pipe] {BioMapper Enrichment \\ (MyGene.info API Query)};
    \node (resp) [io, below=of bio] {HTTP JSON Response \\ (predictions + confidence + biology)};
    
    \draw [arrow] (req) -- (api);
    \draw [arrow] (api) -- (align);
    \draw [arrow] (align) -- (pipe);
    \draw [arrow] (pipe) -- (bio);
    \draw [arrow] (bio) -- (resp);
\end{tikzpicture}
\caption{Architectural flowchart of the FastAPI inference and biological enrichment service.}
\label{fig:api_architecture}
\end{figure}

\begin{table*}[htbp]
\centering
\caption{FastAPI Endpoint Routing and JSON Schema Mappings}
\label{tab:api_endpoints}
\begin{tabular}{llllp{6.5cm}}
\toprule
\textbf{Endpoint} & \textbf{Method} & \textbf{Input Schema Class} & \textbf{Output Schema Class} & \textbf{JSON Payload Structure} \\
\midrule
/health & GET & None & Dict & \texttt{\{"status": "healthy", "pipeline\_loaded": true\}} \\
\midrule
/predict & POST & InferenceRequest & PredictionResponse & Request: \texttt{\{"samples": [\{"LDB2": 1.2, "SLIT3": 0.8, \dots\}]\}} \\
         &      &                  &                    & Response: \texttt{\{"results": [\{"prediction": "1", "confidence": 0.95, "biological\_insight": \{\dots\}\}], "status": "success"\}} \\
\bottomrule
\end{tabular}
\end{table*}
