...
Moving machine learning frameworks into diagnostic labs involves regulatory challenges. In the United States, models must satisfy Food and Drug Administration (FDA) guidelines for Software as a Medical Device (SaMD)~\cite{samd}. This requires:
\begin{enumerate}
    \item \textbf{Analytical Validation:} Proving that the model measures transcriptomic expression accurately across platforms.
    \item \textbf{Clinical Validation:} Demonstrating that the model identifies cancer presence accurately in real-world clinical cohorts (e.g., satisfying Clinical Laboratory Improvement Amendments (CLIA) and College of American Pathologists (CAP) certification requirements)~\cite{clia}.
